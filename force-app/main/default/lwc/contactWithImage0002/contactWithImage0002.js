...
//ends here
  ...
  // hasContacts
 @track hasContactss = true;

 @wire(getContacts, { searchval01: '$myValue', searchval02: '$selectedValue', searchval03: '$mylocationvalues' })
wiredGetContacts({ error, data }) {
  if (data) {
    this.contacts = data;
    this.hasContactss = this.contacts.length > 0;
    console.log('length as of data: ' + this.contacts.length);
  } else if (error) {
    this.contacts = [];
    this.hasContactss = this.contacts.length > 0; // Set hasContactss to false when there's an error and contacts is empty
    console.log('the length was: ' + this.contacts.length);
  }
}
  ...
  get contactsWithImageUrl() {
    if (this.hasContacts) {
      return this.contacts.map((contact) => ({
        ...contact,
        imageUrl: contact.imageFileType && contact.imageData ? `data:image/${contact.imageFileType};base64,${contact.imageData}` : 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_640.png'
      })
      );
    } else {
      return [];
  ...
  handleConfirmAppointment() {
    ...
    console.log('selectedDate  ',this.currentDate)
